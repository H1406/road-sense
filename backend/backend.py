...
CLS_MODEL_PATH = r"d:\Khue\Theme 1\runs\rubbish_cls\weights\best.pt"
SEG_MODEL_PATH = r"d:\Khue\Theme 1\runs\rubbish_yolo11l_seg_12cls\weights\best.pt"
CLS_THRESH     = 0.93
SEG_CONF       = 0.15  
...
async def predict(file: UploadFile = File(...)):
    ...
    cls_results = cls_model.predict(
        source=img, imgsz=224, verbose=False, device=0
    )
    ...
    seg_results = seg_model.predict(
        source=img, conf=SEG_CONF, iou=0.4, verbose=False, device=0
    )
    r     = seg_results[0]
    boxes = r.boxes
